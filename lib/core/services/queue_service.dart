...
import '../../../features/player/data/models/player_enums.dart';
...
class QueueService {
  ...
  MediaItem? get nextTrack {
    ...
    if (_isShuffleEnabled) {
      ...
      int nextShuffleIndex = _shuffleQueueIndex + 1;
      if (nextShuffleIndex >= _shuffleQueue.length) {
        // Infinite shuffle: regenerate
        return _peekNextShuffleTrack();
      }
      final nextId = _shuffleQueue[nextShuffleIndex];
  ...
  int getPreviousIndex() {
    ...
    if (_isShuffleEnabled) {
      ...
      // If at start of shuffle, just stay or go to start of list? 
      // Spotify usually just stays at first song of shuffle.
  ...
  void _regenerateShuffleQueue() {
    ...
    // Prevent same song twice if regenerating
