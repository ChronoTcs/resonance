...
class QueueService {
  ...
  int getNextIndex({bool fromCompletion = false}) {
    print('QueueService.getNextIndex: fromCompletion=$fromCompletion, _currentIndex=$_currentIndex, _queue.length=${_queue.length}, _loopMode=$_loopMode');
    if (_queue.isEmpty) return -1;

    // Repeat One logic: only repeat if it's an auto-advance (completion)
    if (fromCompletion && _loopMode == LoopMode.one) {
  ...
  MediaItem? getNextTrack(LoopMode loopMode, bool isShuffleEnabled, {bool fromCompletion = false}) {
    ...
    print('QueueService.getNextTrack: nextIndex=$nextIndex');
    if (nextIndex != -1) {
