...
// ── Windows-only Stream Resolver ─────────────────────────────────────────────
// Uses yt-dlp via Python IPC (DownloadService.resolveStreamUrl).
// Owns the canonical Windows User-Agent strings for all client types.
// NO Android / libmpv imports allowed in this file.
class WindowsStreamResolver implements IPlatformStreamResolver {
  final Ref _ref;

  WindowsStreamResolver(this._ref);

  @override
  String get platformName => 'windows';

  // ── Canonical Windows UA Strings (single source of truth) ─────────────────
  // These match the signatures embedded in yt-dlp CDN URLs.
  ...
  Future<StreamResolutionResult> resolveStream(String videoId) async {
    ...
    if (resolvedUrl == null || resolvedUrl.isEmpty) {
      throw Exception('[WindowsStreamResolver] yt-dlp IPC returned null for $videoId');
    }

    debugPrint('[WindowsStreamResolver] Resolved $videoId via yt-dlp IPC');
